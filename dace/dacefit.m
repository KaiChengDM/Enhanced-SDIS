...
if  nargin > 5
  ...
  log_ub = log10(upb); log_lb = log10(lob); 
  for i = 1:10  % Multistart for hype-parameter optimization
     theta0 = 10.^(rand.*ones(1,n).*(log_ub-log_lb)+log_lb);
     % n
     [thetai(i,:), fi(i), fiti{i}, perfi{i}] = boxmin(theta0, lob, upb, par);
  end
...
obj = m*log(sigma2) + 2*sum(log(diag(C)));
...
if  ~isinf(f)
  % Iterate
  p = length(t);
  if  p <= 2,  kmax = 8; else,  kmax = min(p,8); end
  for  k = 1 : kmax
    th = t;
